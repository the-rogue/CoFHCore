...
public class WorldGenMinableCluster extends WorldGenerator {
	...
	public boolean generate(World world, Random rand, BlockPos pos) {
		...
		if (blocks < 4) { // HACK: at 1 and 2 no ores are ever generated. at 3 only 1/3 veins generate
			return generateTiny(world, rand, pos);
		}
	...
	public boolean generateTiny(World world, Random random, BlockPos pos) {

		boolean r = false;
		// not <=; generating up to clusterSize blocks
		for (int i = 0; i < genClusterSize; i++) {
			BlockPos pos2 = new BlockPos(pos).add(random.nextInt(2), random.nextInt(2), random.nextInt(2));
	...
	public static boolean generateBlock(World world, BlockPos pos, WeightedRandomBlock[] mat, List<WeightedRandomBlock> o) {

		if (mat == null || mat.length == 0) {
			return generateBlock(world, pos, o);
		}

		if (canGenerateInBlock(world, pos, mat)) {
			return generateBlock(world, pos, o);
		}
		return false;
	}
	...
	public static boolean generateBlock(World world, BlockPos pos, List<WeightedRandomBlock> o) {

		WeightedRandomBlock ore = selectBlock(world, o);
		if (ore == null) {
			return false;
		}
		return world.setBlockState(pos, ore.block.getStateFromMeta(ore.metadata), 2);
	}
